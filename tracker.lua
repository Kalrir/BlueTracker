...
local NM_COL     = { 0.96, 0.80, 0.35, 1.0 }       -- gold, for NM monster names
local NM_TAG     = '  [NM]'
local function IsNM(v) return v == true end        -- row[4]/m[4] flag
            ...
            if rows then
                ...
                for _, row in ipairs(rows) do
                    if Canon(row[3]) == zoneName then
                        -- carry zone (row[3]) and NM flag (row[4]) through so the
                        -- tracker/zone views can show the NM marker.
                        mobs[#mobs + 1] = { row[1], row[2], row[3], row[4] }
                    end
                end
                if #mobs > 0 then
...
local function DrawDetailPanel()
    ...
    if rows and #rows > 0 then
        ...
        for _, row in ipairs(rows) do
            local nm = IsNM(row[4])
            imgui.PushStyleColor(ImGuiCol_Text, nm and NM_COL or { 0.82, 0.85, 0.94, 1.0 })
            imgui.Text(row[1] .. (nm and NM_TAG or '')); imgui.NextColumn()
            imgui.PopStyleColor(1)
...
local function TrackerEntry(r, bluLvl, skill, mobs, show_zone, zone_name)
    ...
    if show_zone then
        ...
        for _, m in ipairs(rows) do
            ...
            local mm = tw(m[1] .. (IsNM(m[4]) and NM_TAG or '')); if mm > maxM then maxM = mm end
        end
        local mob_x = maxA + 16
        ...
        for _, m in ipairs(rows) do
            ...
            local nm = IsNM(m[4])
            imgui.PushStyleColor(ImGuiCol_Text, nm and NM_COL or { 0.78, 0.80, 0.90, 1.0 })  -- Mob
            imgui.Text(m[1] .. (nm and NM_TAG or ''))
            imgui.PopStyleColor(1)
...
local function RenderZoneTab()
    ...
    for _, m in ipairs(rows) do
        ...
        local nm = IsNM(m[4])
        imgui.PushStyleColor(ImGuiCol_Text, nm and NM_COL
                                            or (hasPos and { 0.82, 0.85, 0.94, 1.0 }
                                                        or  { 0.55, 0.58, 0.70, 1.0 }))
        imgui.Text(m[1] .. (nm and NM_TAG or '') .. (hasPos and '' or '  (no map)')); imgui.NextColumn()
        imgui.PopStyleColor(1)
        imgui.PushStyleColor(ImGuiCol_Text, { 0.70, 0.73, 0.85, 1.0 })
