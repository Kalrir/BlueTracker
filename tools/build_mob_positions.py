...
ROW_RE = re.compile(
    ...
    r"(\d+),"          # mobid
    r"(\d+),"          # spawnslotid
    r"'((?:[^'\\]|\\.)*)',"   # mobname (underscored; backslash-escape safe)
    r"'((?:[^'\\]|\\.)*)',"   # polutils_name (spaced; backslash-escape safe)
    r"(\d+),"          # groupid
    r"(\d+),"          # minLevel
    r"(\d+),"          # maxLevel
    r"([-0-9.]+),"     # pos_x
...
def display_name(polutils: str, mobname: str) -> str:
    ...
    n = _unesc(polutils).strip()
    if not n:
        n = _unesc(mobname).replace("_", " ").strip()
    return n
...
def dedup(points):
    """Merge near-identical spawns. Points are (x, y, z). Height (y) is part of
    the key so vertically stacked spawns (same x/z, different floor) are NOT
    merged -- that distinction is what lets the tracker separate stacked floors."""
    out = []
    for p in points:
        keep = True
        for q in out:
            if (abs(p[0] - q[0]) < DEDUP_EPS
                    and abs(p[2] - q[2]) < DEDUP_EPS
                    and abs(p[1] - q[1]) < DEDUP_EPS):
                keep = False
                break
        if keep:
            out.append(p)
...
def main():
    ...
    with open(sql_path, encoding="utf-8", errors="replace") as fh:
        for line in fh:
            ...
            if not is_placed(x, y, zz):
                continue
            zc = zones[z]
            zc["mobs"].setdefault(name, []).append((x, y, zz))   # (x, height, z)
            zc["minx"] = min(zc["minx"], x); zc["maxx"] = max(zc["maxx"], x)
            zc["minz"] = min(zc["minz"], zz); zc["maxz"] = max(zc["maxz"], zz)

    # Build one Lua block per zone (as `M["Zone"] = { ... }` assignments).
    zone_blocks = []
    for z in zone_ids:
        zc = zones[z]
        ...
        b.append('    M["%s"] = {' % name)
        b.append('        zoneid = %d,' % z)
        b.append('        bounds = { min_x = %s, max_x = %s, min_z = %s, max_z = %s },'
                 % (lua_num(minx), lua_num(maxx), lua_num(minz), lua_num(maxz)))
        sx, ox, sy, oy, ref = fit_calibration(zc["minx"], zc["maxx"], zc["minz"], zc["maxz"])
        b.append('        -- Fitted world->map calibration (Boussole "direct" format).')
        b.append('        -- Replace with your client values / a boussole_custom_maps entry')
        b.append('        -- to align a real map image (see README). image = optional PNG in maps/.')
        b.append('        calibration = { scalingX = %s, offsetX = %s, scalingY = %s, offsetY = %s, referenceSize = %d, image = "%d_0.png" },'
                 % (lua_num(sx), lua_num(ox), lua_num(sy), lua_num(oy), ref, z))
        b.append('        mobs = {')
        for mob in sorted(mobs.keys()):
            pts = dedup(mobs[mob])
            inner = ", ".join("{x=%s,y=%s,z=%s}" % (lua_num(px), lua_num(py), lua_num(pz))
                              for px, py, pz in pts)
            b.append('            ["%s"] = { %s },' % (mob.replace('"', '\\"'), inner))
        b.append('        },')
        b.append('    }')
        block = "\n".join(b)
    ...
    # LuaJIT / Lua 5.1 cap a single function at 65536 constants. A flat
    # `return { ... }` for the whole game blows past that, so pack the zone
    # assignments into several loader functions, each kept well under budget.
    def literal_count(t):
        return len(re.findall(r'-?\d+\.?\d*(?:[eE]-?\d+)?', t)) + t.count('["')

    BUDGET = 28000
    chunks, cur, curc = [], [], 0
    for block in zone_blocks:
        c = literal_count(block)
        if cur and curc + c > BUDGET:
            chunks.append(cur); cur, curc = [], 0
        cur.append(block); curc += c
    if cur:
        chunks.append(cur)
    ...
    out.append("-- sql/mob_spawn_points.sql. World coords: +x=east, +z=south, y=height.")
    out.append("-- Each point is {x=,y=,z=}; y (height) is used to separate stacked floors.")
    out.append("-- Contains MOB POSITIONS ONLY (no client map artwork).")
    out.append("-- Zones are split across _loadN(M) functions so no single Lua function")
    out.append("-- exceeds the LuaJIT/Lua 5.1 limit of 65536 constants.")
    out.append("")
    out.append("local M = {}")
    out.append("")
    for i, ch in enumerate(chunks, 1):
        out.append("local function _load%d(M)" % i)
        out.append("\n".join(ch))
        out.append("end")
        out.append("")
    out.append("".join("_load%d(M) " % i for i in range(1, len(chunks) + 1)).rstrip())
    out.append("return M")
    sys.stdout.write("\n".join(out) + "\n")
